...
public class UserController {
    ...
    @Operation(
            ...
            description = "Ye API ek user ko uske ID ke basis pe return karti hai"
    )
    @ApiResponse(responseCode = "200", description = "User details successfully fetched")
    @ApiResponse(responseCode = "404", description = "User not found")
    @GetMapping("/{id}")
    public User find(@Parameter(
            description = "User ID jisse fetch karna hai") @PathVariable Long id) {
